...
### Die Lied-Ausgabe {#sec-lieder}

Mit dem nächsten Befehl wird die Lied-Ausgabe bewirkt: `\songs`.

::: callout-tip
Alle sich im Liederverzeichnis befindenden Lieder können mit folgendem Code inkludiert werden:

``` lilypond
% alle Lieder auf einmal reinhaun:
#(for-each (lambda (song) (includeSong song)) (sort-list (files-in-directory songPath) string-ci<?))
```

Der Befehl kann an die Stelle geschrieben werden, wo sonst die einzelnen Lieder referenziert werden würden.
:::

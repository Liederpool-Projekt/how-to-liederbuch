...
Alternative Titel können mittels der Variable `alttitle = ""` angegeben werden, wobei innerhalb der Anführungszeichen der Alternativtitel benannt wird. Es kann auch mehrere alternative Titel geben, dafür kann einfach eine weitere Variable mit der Bezeichnung `altalttitle` genutzt werden. Bei weiteren Titeln dann `altaltalttitle` und so weiter. Es sind bis zu drei Alternativtitel vorgesehen, bei Bedarf könnte man die aber auch noch erweitern.
